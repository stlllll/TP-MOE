...
if __name__ == '__main__':
    ...
    pred_m = np.empty((0,1))
    pred_v = np.empty((0,1))
    pred_log_likelihood = np.empty((0,1))
    runtime = []

    for i in range(1,N,1):
        m, v = tpmoe.predict(X[i,None])
    ...
    #To reproduce all the results in paper
    #Brent: truncate the first 100 datapoints
    #Canada: all
    #Nile: all
    #EUR-USD: truncate the first 200 datapoints
    #supplementary materials:
    #Dow: all
    #Istanbul: pick up every 5 data point
    #foreign exchange rates: pick up every 50 data point
    #DJI: pick up every 10 data points
